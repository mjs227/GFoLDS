...
ap = ArgumentParser()
ap.add_argument('-g', '--gfolds', action='store_true')
ap.add_argument('-b', '--bert_model_type', help='\'base\' or \'large\'', type=str, default=None)
ap.add_argument('-c', '--comp', help='BERT comparison model (original otherwise)', action='store_true')
ap_args = ap.parse_args()

if ap_args.bert_model_type is not None:
    if ap_args.gfolds:
        raise ValueError('Specify either \'--gfolds\' or \'--bert_model_type\' (but not both)')

    FP = f'{GLOBAL_FP}/prop_inf/mcrae_data_bert{"_comp" if ap_args.comp else ""}_{ap_args.bert_model_type}.pkl'
elif ap_args.gfolds:
    FP = f'{GLOBAL_FP}/prop_inf/mcrae_data_gfolds.pkl'
else:
    raise ValueError('Specify either \'--gfolds\' or \'--bert_model_type\' (but not both)')
