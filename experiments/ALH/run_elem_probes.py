...
from create_data import TOK_FP, TRGT_FP_POS, TRGT_FP_QUANT, BERT_TKNZR, POS_MAP, SINGULAR, PLURAL, BOTH, get_pos
...
def get_prec_print_fn_pos(model_type):
    ...
    if bert_tokenizer_p is None:
        bert_tokenizer_p = AutoTokenizer.from_pretrained(BERT_TKNZR)

    if model_type == 'gfolds':
        global gfolds_toks_rev_p

        if gfolds_toks_rev_p is None:
            gfolds_toks_rev_p = _get_gfolds_toks_rev(TOK_FP)
...
def get_prec_print_fn_quant(model_type):
    ...
    if bert_tokenizer_q is None:
        bert_tokenizer_q = AutoTokenizer.from_pretrained(BERT_TKNZR)

    if model_type == 'gfolds':
        global gfolds_toks_rev_q

        if gfolds_toks_rev_q is None:
            gfolds_toks_rev_q = _get_gfolds_toks_rev(TOK_FP)
...
def init_gfolds(pos):
    ...
    gfolds_toks = _get_gfolds_toks_rev(TOK_FP, incl_toks=(not pos))
    bert_tokenizer = AutoTokenizer.from_pretrained(BERT_TKNZR) if PRINT_TOP else None
...
def init_pos():
    ...
    with open(TRGT_FP_POS, 'r') as f_data:
...
def init_quant():
    ...
    with open(TRGT_FP_QUANT, 'r') as f_data:
        data_file = json.load(f_data)
...
ap = ArgumentParser()
ap.add_argument('-p', '--probe_type', help='\'pos\' or \'quantifier\'', type=str, default='pos')
ap.add_argument('-g', '--gfolds_run', help='Format: run_{n}/ep{k}_mb{i}.chk', type=str, default=None)
ap.add_argument('-b', '--bert_run', help='Format: run_{n}/ep{k}_mb{i}.chk', type=str, default=None)
ap.add_argument('--bert_model_type', help='\'base\' or \'large\'', type=str, default='base')
ap_args = ap.parse_args()

PROBE = ap_args.probe_type
assert PROBE.strip().lower() in {'pos', 'quantifier'}

BERT_CHK_FP, BERT_MODEL, GFOLDS_CHK_FP, RESULTS_FP, = None, None, None, None
PRINT_TOP = False
PREC_CNT = 10  # POS only
DEVICE = 0 if torch.cuda.is_available() else 'cpu'

if ap_args.gfolds_run is None:
    if ap_args.bert_run is None:
        raise ValueError('Specify either \'--gfolds_run\' or \'--bert_run\' (but not both)')

    BERT_CHK_FP = f'{GLOBAL_FP}/bert/runs/{ap_args.bert_run}/checkpoints/'
    BERT_MODEL = ap_args.bert_model_type
    RESULTS_FP = f'{GLOBAL_FP}/elem_eval/stats/{BERT_MODEL}/{PROBE}.json'
elif ap_args.bert_run is None:
    GFOLDS_CHK_FP = f'{GLOBAL_FP}/pretraining/{ap_args.gfolds_run}/checkpoints/'
    RESULTS_FP = f'{GLOBAL_FP}/elem_eval/stats/gfolds/{PROBE}.json'
else:
    raise ValueError('Specify either \'--gfolds_run\' or \'--bert_run\' (but not both)')
