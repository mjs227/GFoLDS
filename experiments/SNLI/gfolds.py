...
from experiments.SNLI.bert import lr_step
...
if ap_args.pretrained_checkpoint is None:
    INIT_FROM_PT_CHECKPOINT = None
else:
    INIT_FROM_PT_CHECKPOINT = '{}/pretraining/{}/checkpoints/{}'.format(
        GLOBAL_FP, *ap_args.pretrained_checkpoint.split('/')
    )
...
if INIT_FROM_PT_CHECKPOINT is None:  # load from fine-tuned checkpoint
    ...
    print(f'INITIALIZING FROM PRE-TRAINED CHECKPOINT {ap_args.pretrained_checkpoint} ...')
    model, max_chk = SWATForSequenceClassification.from_swat_model(INIT_FROM_PT_CHECKPOINT, MODEL_CONFIG), -1

    model.to(device=DEVICE)
    optimizer = DualOptimizer(model, OPTIMIZER_LM_KWARGS)
