...
FILEPATH, RUN = f'{GLOBAL_FP}/bert/', 0
CHECKPOINT_FILEPATH = FILEPATH + f'runs/run_{RUN}/checkpoints/'
OPTIMIZER_FILEPATH = FILEPATH + f'runs/run_{RUN}/optimizers/'
BASE_DATA_FILEPATH = FILEPATH + 'data/mlm/batch_data/0/'  # no effect if RANDOMIZE_BATCHES = False
BATCH_FILEPATH = FILEPATH + f'data/mlm/batch_data/'  # empty if RANDOMIZE_BATCHES, else folders {0, ..., N_EPOCHS}
STATS_FILEPATH = FILEPATH + f'runs/run_{RUN}/stats/'

BERT_MODEL = 'bert-base-uncased'
MODEL_TO = {'device_map': 'auto'}
...
PRINT_OUTPUT = False  # print loss
TQDM = False  # tqdm train loop---no effect if PRINT_OUTPUT = FALSE
