...
def generate_batches(mb_file):
    for i in range(0, len(mb_file), cfg.BATCH_SIZE):
        max_size = max(len(x[1]) for x in mb_file[i:i + cfg.BATCH_SIZE])
        inputs, mask_targets, s_idx_list, nsp_targets = [], [], [], []

        for sent, s_idxs_, nsp_lbl in mb_file[i:i + cfg.BATCH_SIZE]:
            x_in, x_trgt = perturb_bert(torch.tensor(sent, dtype=cfg.TOKEN_DTYPE))
            x_idx = torch.tensor(s_idxs_, dtype=cfg.TOKEN_DTYPE)

            if x_idx.size(-1) < max_size:
                size_diff = max_size - x_idx.size(-1)
                ...
                x_idx = torch.cat((x_idx, torch.full((size_diff,), 1, dtype=cfg.TOKEN_DTYPE)), dim=0)

            inputs.append(x_in.unsqueeze(0))
            mask_targets.append(x_trgt.unsqueeze(0))
            s_idx_list.append(x_idx.unsqueeze(0))
            nsp_targets.append(nsp_lbl)

        batch_input = torch.cat(inputs, dim=0).to(device=DEVICE)
        batch_s_idxs = torch.cat(s_idx_list, dim=0).to(device=DEVICE)
        batch_mask_trgt = torch.cat(mask_targets, dim=0).to(device=DEVICE).type(LONG_TENSOR)
        batch_nsp_trgt = torch.tensor(nsp_targets, dtype=cfg.TOKEN_DTYPE, device=DEVICE).type(LONG_TENSOR)
        batch_attn_mask = (batch_input != cfg.PAD_ID).to(dtype=cfg.TOKEN_DTYPE)

        yield batch_input, batch_s_idxs, batch_mask_trgt, batch_nsp_trgt, batch_attn_mask


def generate_nsp_data(mb_file):
    all_sents, len_buckets = [x[0] for x in mb_file], [[] for _ in range(508)]
    all_sents.extend(x[1] for x in mb_file)
    out_data = []

    for i, sent in enumerate(all_sents):
        len_buckets[(509 - len(sent)) - 1].append(i)

    for i in reversed(range(1, 508)):
        len_buckets[i - 1].extend(len_buckets[i])

    for item in mb_file:
        if random.random() < 0.5:
            nsp_target, s1, s2 = 0, None, None

            if len(len_buckets[len(item[0]) - 1]) > 0:
                s1 = item[0]
            elif len(len_buckets[len(item[1]) - 1]) > 0:
                s1 = item[1]
            if s1 is not None:
                s2 = all_sents[random.choice(len_buckets[len(s1) - 1])]
        else:
            nsp_target, (s1, s2) = 1, item

        if s1 is not None:
            s_idxs_ = [0 for _ in range(len(s1) + 2)] + [1 for _ in range(len(s2) + 1)]  # +2/1 for [CLS]/[SEP]
            out_data.append(([101] + s1 + [102] + s2 + [102], s_idxs_, nsp_target))

    del all_sents, len_buckets
...
model = AutoModelForPreTraining.from_pretrained(cfg.BERT_MODEL)
...
try:
    for ep in range(chk_ep, cfg.N_EPOCHS):
        for meta_batch in range(chk_mb, n_batches):  # meta-training loop
            ...
            meta_batch_nsp = generate_nsp_data(meta_batch_file)
            batches = generate_batches(meta_batch_nsp)
            ...
            for _ in tqdm_fn(range(-(-len(meta_batch_file) // cfg.BATCH_SIZE))):  # main training loop
                try:
                    toks, s_idxs, mask_trgt, nsp_trgt, attn_mask = next(batches)
                    model_out = model(
                        toks,
                        attention_mask=attn_mask,
                        token_type_ids=s_idxs,
                        labels=mask_trgt,
                        next_sentence_label=nsp_trgt
                    )
                    loss = model_out.loss
                    ...
                    optimizer.zero_grad()
                    del toks, s_idxs, mask_trgt, nsp_trgt, attn_mask, model_out, loss
