...
FILEPATH, RUN = '/projects/academic/rchaves/michael_diss/', 0
CHECKPOINT_FILEPATH = FILEPATH + f'run_{RUN}/checkpoints/'
OPTIMIZER_FILEPATH = FILEPATH + f'run_{RUN}/optimizers/'
BASE_DATA_FILEPATH = FILEPATH + 'data/'  # no effect if RANDOMIZE_BATCHES = False
BATCH_FILEPATH = FILEPATH + f'run_{RUN}/batch_data/'  # empty if RANDOMIZE_BATCHES, else folders {0, ..., N_EPOCHS}
STATS_FILEPATH = FILEPATH + f'run_{RUN}/stats/'
LOSS_WEIGHT_FILE = None

MODEL_CONFIG = cfg.SWATForMaskedLMConfig(
